...
/**
 * A checkout. beneficiaryWardId is set when a parent orders for a ward (the current
 * flow); beneficiaryStudentProfileId is the older link-an-existing-student flow, kept
 * only for historical/older-client compatibility and no longer reachable from either
 * frontend's UI. idempotencyKey is a client-generated id for this checkout — resending
 * it returns the same order instead of placing another. There is a single ordering slot
 * (Recess), so the caller no longer chooses one — the server resolves it automatically.
 */
